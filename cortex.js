...
module.exports = class Cortex {
    ...
    ready() {
        return new Promise((resolve, reject) => {
            this.socket.onopen = () => {
                console.log("Connected to the Cortex WebSocket!")
                resolve()
            }

            this.socket.onerror = error => {
                console.log("Error connecting to the Cortex WebSocket:", error)
                reject(error)
            }
        })
    }
    ...
    subRequest(stream, authToken, sessionId){
        ...
        socket.on('message', (data)=>{
            ...
            // cortex first returns response to the request
            if (jsonData.hasOwnProperty('id') && jsonData.id == SUB_REQUEST_ID) {
                const successStreams = jsonData.result.success
                if (successStreams.length = 1 && successStreams[0].streamName == 'eeg') {
            ...
            // then it starts sending samples through
            try {
                console.log(jsonData.eeg)
            } catch(error) {
                console.log("Error parsing sample:", error)
            }
        })
    }
